...
public class PSO_FunctionTest_12x12 : MonoBehaviour
{
    public double[] xlimit = new double[2] { 0, 18 };       //位置边界设置
    public double[] ylimit = new double[2] { 0, 20 };
    public double[] vlimit = new double[2] { -2, 2 };       //速度边界设置
    public int experimentNumber= 1;
    public int generation = 200;             //迭代次数
    ...
    private static double[] Frequent_data =
                                     {0,14,0,17,22,5,0,3,22,5,10,12,
                                      22,0,12,8,0,2,5,8,2,0,8,13,
                                      12,9,0,18,6,0,8,0,18,25,6,18,
                                      0,21,14,0,8,33,0,12,0,6,12,4,
                                      8,5,12,10,0,9,20,8,0,11,8,6,
                                      15,2,11,6,5,0,0,8,22,6,12,8,
                                      12,8,6,21,9,0,0,12,8,8,9,2,
                                      4,18,22,10,0,0,12,0,10,0,2,4,
                                      5,22,9,0,12,8,2,0,0,8,12,5,
                                      22,10,6,18,6,8,0,2,12,0,8,4,
                                      0,8,5,9,27,35,25,0,0,9,0,12,
                                      8,0,8,14,22,6,14,0,12,12,8,0};//物流运输频次
    private static double Cost_per_meter = 1;//物流每米花费
    private double[] Equipment_Length =
        {1,1.5,1.5,1.2,1.5,2.5,2.5,1.2,1.2,1.2,1.0,1.0};//设备长度
    private double[] Equipment_Width =
        {0.8,0.8,0.8,1.0,0.8,0.8,0.8,1.0,1.0,1.0,0.8,0.8};//设备宽度
    private double Equipment_Min_Length = 0.5;//设备间最小长度 Hij
    private double Equipment_Min_Width = 0.5;//设备间最小宽度 Vij
    private double Wall_Min_Length = 0.5;//设备间最小长度  H0i
    private double Wall_Min_Width = 0.5;//设备间最小宽度   V0i
    ...
    private void Generate_Layout_Plan(double[] x_gbest) 
    {
        ...
        for (int i = 0; i < equipmentNumber; i++)
        {
            ...
            if (machineGameObject.Count() == 0 || machineGameObject[i] == null)
            {
