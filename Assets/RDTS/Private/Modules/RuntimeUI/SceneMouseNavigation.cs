...
namespace RDTS
{
    ...
    public class SceneMouseNavigation : RDTSBehavior
    {
        ...
        public FirstPersonController FirstPersonController;//第一人称控制脚本
        ...
        private bool touch;
        private bool cinemachineactive = false;//虚拟相机
        ...
        * LateUpdate 上的相机逻辑仅在处理完所有角色移动逻辑后才更新。
        */
        void LateUpdate()
        {


            if (FirstPersonControllerActive)//在第一人称视角下则不执行
                return;
            ...
            else if (Input.GetMouseButton(1) && !touch)
            {
                ...
                // set camera rotation 
                desiredRotation = Quaternion.Euler(yDeg, xDeg, 0);//y轴上的增量即绕x轴旋转，x轴上的增量即绕y轴旋转
                currentRotation = transform.rotation;
            ...
            // 否则，如果选择了鼠标中键，我们将通过在屏幕空间中转换目标来平移
