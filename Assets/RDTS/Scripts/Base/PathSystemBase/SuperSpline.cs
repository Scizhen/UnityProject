...
/// 运输机系统中，该脚本的提供了创建、计算和渲染样条曲线所需的最重要的函数
/// </summary>
/// <remarks>
...
/// ///它提供了创建、计算和渲染样条曲线所需的最重要的函数。
///类派生自MonoBehaviour，所以它可以附加到gameObjects，并像任何其他自己编写的脚本一样使用。
/// </remarks>
///
[ExecuteInEditMode]
...
public partial class SuperSpline : MonoBehaviour
{
    ...
    private bool HasNodes
    {
        get
        {
            if (splineNodesInternal == null || splineNodesInternal.Count == 0)
            {
                splineNodesInternal = new List<SplineNode>();
                splineNodesInternal.AddRange(splineNodesArray.GetRange(0, splineNodesArray.Count));
                splineNodesInternal.Remove(null);
                ReparameterizeCurve();
            }
            return splineNodesInternal.Count > 0;
        }
    }
    ...
    public enum TangentMode
    {
        ...
        UseTangents,            // Use te vector that connects the two adjacent control nodes as tangent.
