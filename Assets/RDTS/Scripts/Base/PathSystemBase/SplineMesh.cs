...
{
    public SuperSpline spline;                                              // Reference to the spline that defines the path.
    ...
    public Vector3 Offset = Vector3.zero;                               // Mesh offset
    ...
    void Start()
    {
        if (updateMode != UpdateMode.DontUpdate)
        {
            if (spline != null)
                spline.UpdateSpline();
            UpdateMesh();
...
#if UNITY_EDITOR
    private void UnwrapMesh(Mesh mesh)
    {
        Vector2[] array = Unwrapping.GeneratePerTriangleUV(mesh);
        MeshUtility.SetPerTriangleUV2(mesh, array);
        mesh.uv = mesh.uv2;
    }
#endif

    private void MoveMesh(MeshData meshDataBase, Vector3 offset)
    {
        for (int i = 0; i < meshDataBase.VertexCount; i++)
        {
            meshDataBase.vertices[i] += offset;
        }
    }
    ...
    private void SetupMesh()
    {
        ...
        bentMesh.hideFlags = HideFlags.None;
    ...
    public enum UVMode
    {
        ...
        Wrap // UV coordinates will simply be copied from the base mesh and won't be altered.
    }
