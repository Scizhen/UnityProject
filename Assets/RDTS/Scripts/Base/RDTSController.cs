...
{
    [ExecuteAlways]
    //! This object needs to be in every Parallel-RDTS scene. It controls main central data (like scale...) and manages main RDTS settings for the scene.
    //! 这个对象需要在每个 Parallel-RDTS 场景中。 它控制主要的中心数据（如缩放大小...）并管理场景的主要 RDTS 设置。
    public class RDTSController : RDTSBehavior
    {
        ...
        [OnValueChanged("ChangedTimeScale")]
        [Range(0, 20)]
        ...
        [EnableIf("StandardBasePlateAndLights")]
        [OnValueChanged("ChangedVisual")]
        ...
        public void OnConnectionButtonPresed(GenericButton button)
        {
            ...
            var objs = UnityEngine.Resources.FindObjectsOfTypeAll<RDTSBehavior>();
            foreach (var obj in objs)
            {
        ...
        public void OnPlayModeFinished()
        {
            ...
            // BroadcastAll("OnEnteredEditMode");
#endif
        }
        ...
        public void OnEnable()
        {
            ...
            {
                // After End of Play
                QuickToggle.SetRDTSController(this);
                UpdateAllLockedAndHidden();
            }
        ...
        /// <summary>
        /// 通过控制器脚本更新Hierarchy面板中signal（值对象或Value类型脚本）的 数据连接信息（ValueConnectionInfo折叠菜单中的信息）
        /// </summary>
        private void UpdateSignals()
        {
            /// Clear Info on all Signals
            var signals = FindObjectsOfType<Value>();
            foreach (var signal in signals)
            {
                signal.DeleteValueConnectionInfos();
            }

            /// get all Behavior models
            var behaviors = FindObjectsOfType<MonoBehaviour>().OfType<IValueInterface>();//返回所有类型为 MonoBehaviour 的已加载的激活对象，并从中筛选出IValueInterface类的对象
            foreach (var behavior in behaviors)//遍历所有挂载在此场景中gameobject上且继承IValueInterface接口的脚本
            {
                // now get all signals in behaviors 获取所有附带Value类型字段的信息
                var connections = behavior.GetConnections();
                foreach (var info in connections)
                {
                    if (info.Value != null)//若此信息中的Value类型字段不为空
                    {
                        info.Value.AddValueConnectionInfo(behavior.gameObject, info.Name);//为该字段（对应的Value类型脚本）添加数据连接信息，即Value子类脚本中Inspector面板的ValueConnectionInfo折叠菜单中的信息
                    }
                }
            }
        }
        ...
        new void Awake()
        {
            ...
            Global.rdtsController = this;
            ChangeUIEnable();

            //横距复选框RuntimeInspectorEnabled和UIEnabledOnStart来控制RuntimeInspector的UI显示
            ...
            // Call all G4AAwake
            var behaviors = Object.FindObjectsOfType<RDTSBehavior>();
