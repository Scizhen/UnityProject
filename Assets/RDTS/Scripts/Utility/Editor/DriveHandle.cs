 ...
 namespace RDTS.Utility.Editor
{
#if UNITY_EDITOR
    /// <summary>
    /// Drive脚本的编辑脚本。在编辑和播放模式下，在场景视图中的 Unity 编辑器中显示驱动手柄的类使用驱动手柄，可以定义驱动方向，并且可以在播放模式下手动移动驱动器
    /// </summary>
    ...
    public class DriveHandle : NaughtyAttributes.Editor.NaughtyInspector
    {
        ...
        private RDTSController _settings;
        ...
        protected virtual void OnSceneGUI()
        {
            ...
            if (!_isinit)
            {
               ...
               // _kinematic = drive.GetComponent<Kinematic>();
                _isinit = true;
                _settings = UnityEngine.Object.FindObjectOfType<RDTSController>();
                if (_settings != null)
                {
