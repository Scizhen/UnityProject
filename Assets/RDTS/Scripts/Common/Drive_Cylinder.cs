...
namespace RDTS
{
    ...
    public class Drive_Cylinder : BehaviorInterface
    {
        ...
        [Header("Settings")] public bool OneBitCylinder = false; //!< if set to true only one bit is needed to controll the cylinder. If Out=false the cylinder moves in如果设置为true，只需要一个比特来控制圆柱体。如果Out=false，则圆柱体向内移动
