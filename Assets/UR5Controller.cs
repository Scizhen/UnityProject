...
namespace RDTS.RobotSimulationProgramming
{
    ...
    public class UR5_Solver
    {
        ...
        private float[] DH_d = { 0f, .08916f, 0f, 0f, -.11915f, .1f, .0823f };//link offset
        private float[] DH_a = { 0f, 0f, -.425f, -.39225f, 0f, 0f, 0f };//link length
        private float[] DH_alph = { 0f, (float)(Math.PI / 2), 0f, 0f, (float)(Math.PI / 2), (float)(-Math.PI / 2), 0f };//link twist 
        ...
        private void IK_Solver(Matrix4x4 efPos)
        {
            ...
            //first theta1 calculation with the positive square root 第一个带有正平方根的 theta1 计算
            ...
            //fill in the first four elements of the first row of the solution matrix with theta1 用 theta1 填充解矩阵第一行的前四个元素
            for (int i = 0; i < 4; i++)
            {
                this.solutionMatrix[0, i] = theta1;
            }

            //recalculate theta1 for the negative square root and store 重新计算 theta1 的负平方根并存储
            theta1 = (float)(Math.Atan2(p05.y, p05.x) - Math.Acos(DH_d[4] / Math.Sqrt(Math.Pow(p05.x, 2) + Math.Pow(p05.y, 2))) + Math.PI / 2);
            for (int i = 4; i < 8; i++)
            {
                this.solutionMatrix[0, i] = theta1;
            ...
            //theta5 is negative theta5 为负
            sin5 = (float)Math.Sin(this.solutionMatrix[4, 2]);
            theta6 = (float)Math.Atan2(((-xHat[1] * sin1 + yHat[1] * cos1) / sin5), ((xHat[0] * sin1 - yHat[0] * cos1) / sin5));
            this.solutionMatrix[5, 2] = this.solutionMatrix[5, 3] = theta6;

            //theta1 is negative sqrt and theta5 is positive theta1 为负 sqrt，theta5 为正
            ...
            //theta1 is negative sqrt and theta5 is negative theta1 为负 sqrt，theta5 为负
            ...
            //because thetas 1, 5, and 6 are different in different columns, will need to solve across all columns 由于 thetas 1、5 和 6 在不同列中不同，因此需要在所有列中求解
            for (int i = 0; i < 8; i++)
            {
                ...
                //need to get the magnitude of the translation 需要获得平移的幅度
                p14.x = T14.m03;
                p14.y = T14.m13; //make sure this works as a negative 确保这起到负面作用
                p14.z = T14.m23;
                p14.w = 1;

                p14Norm = (float)Math.Sqrt(Math.Pow(p14.x, 2) + Math.Pow(p14.z, 2));

                //will be negative for odd indices 对于奇数索引将为负
                if (i % 2 == 0)
                {
    ...
    public class UR5Controller : MonoBehaviour
    {
        ...
        public float[] jointValues = new float[6];//显示关节角度值
        ...
        private GameObject[] jointList = new GameObject[6];//获取对应关节的对象
        ...
        void LateUpdate()
        {
            ...
            if (userHasHitOk)
            {
                ...
                for (int j = 0; j < 6; j++)
                {
                    ...
                    //用Drive脚本实现
                    Axis[j].CurrentPosition = jointValues[j] = Robot11.solutionArray[j];
                }
