...
public class RelayNode : BaseNode
{
    ...
    public void GetInputs(List<SerializableEdge> edges)
    {
        ...
        // If the relay is only connected to another relay:  若中继节点只连接到另一个中继节点
        ...
        {
            input.values = edges.Select(e => e.passThroughBuffer).ToList();//记录所有连接到的edge的缓冲区值
            input.names = edges.Select(e => e.outputPort.portData.displayName).ToList();//记录所有连接到的edge的显示名称
            input.types = edges.Select(e => e.outputPort.portData.displayType ?? e.outputPort.fieldInfo.FieldType).ToList();//记录显示类型，若显示现象为空则记录字段类型
        }
    }
    ...
    {
        if (inputPorts.Count == 0)//无输入端口，直接返回
            return;

        var inputPortEdges = inputPorts[0].GetEdges();//获取第一个输入端口的所有edge
    ...
    IEnumerable<PortData> InputPortBehavior(List<SerializableEdge> edges)
    {
        ...
        {
            // Add the size of all input edges:  相加所有输入edge的尺寸
