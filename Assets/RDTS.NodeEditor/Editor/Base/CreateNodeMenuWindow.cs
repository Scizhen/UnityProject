...
{
    // TODO: replace this by the new UnityEditor.Searcher package 用新的 UnityEditor.Searcher 包替换它
    /// <summary>
    /// 搜索(窗口)树，有两种形式：右键点击打开、连线时鼠标光标移到空白处打开
    /// </summary>
    class CreateNodeMenuWindow : ScriptableObject, ISearchWindowProvider//ISearchWindowProvider此接口描述了管理图形节点搜索会话的方法
    {
        ...
        {
            // Sort menu by alphabetical order and submenus 按字母顺序和子菜单排序菜单
            ...
            foreach (var nodeMenuItem in nodeEntries)
            {
                ...
                if (parts.Length > 1)
                {
                    ...
                    for (var i = 0; i < parts.Length - 1; i++)
                    {
                        ...
                        // Add section title if the node is in subcategory 如果节点在子类别中，则添加部分标题
        ...
        public bool OnSelectEntry(SearchTreeEntry searchTreeEntry, SearchWindowContext context)
        {
            ...
            var view = graphView.AddNode(BaseNode.CreateFromType(nodeType, graphMousePosition));//添加一个节点
